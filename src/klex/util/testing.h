...
#define TEST_ENV_SETUP(Name)                                                  \
  class _CALLBACK_NAME(Name) : public ::klex::util::testing::Callback {       \
   public:                                                                    \
    void invoke() override;                                                   \
   private:                                                                   \
    static ::klex::util::testing::Callback* const ref_ [[maybe_unused]];      \
  };                                                                          \
                                                                              \
  ::klex::util::testing::Callback* const                                      \
  _CALLBACK_NAME(Name)::ref_ =                                                \
      ::klex::util::testing::UnitTest::instance()->addInitializer(            \
...
#define TEST_ENV_F(EnvName)                                                   \
  ::klex::util::testing::UnitTest::instance()->addEnvironment(                \
      std::unique_ptr<::klex::util::testing::Environment>(EnvName));

// ############################################################################

#define TEST(testCase, testName) _CREATE_TEST(testCase, testName, ::klex::util::testing::Test)
...
#define EXPECT_THROW(program, ExceptionType)                                  \
  do {                                                                        \
    try {                                                                     \
      program;                                                                \
      ::klex::util::testing::UnitTest::instance()->reportEH(                  \
...
#define _EXPECT_BOOLEAN(fileName, lineNo, fatal, expected, actual)            \
  do {                                                                        \
    ...
    if (failed) {                                                             \
      ::klex::util::testing::UnitTest::instance()->reportBinary(              \
...
_TEST_CLASS_NAME(testCaseName, testName)::test_info_ =                        \
    ::klex::util::testing::UnitTest::instance()->addTest(                     \
        #testCaseName, #testName,                                             \
        std::make_unique<                                                     \
            ::klex::util::testing::TestFactoryTemplate<                       \
                _TEST_CLASS_NAME(testCaseName, testName)>>());                \
                                                                              \
