...
GrammarParser::GrammarParser(GrammarLexer&& _lexer, Report* _report):
    report_ { _report }, lexer_ { std::move(_lexer) }
{
}

GrammarParser::GrammarParser(string source, Report* _report):
    report_ { _report }, lexer_ { std::move(source) }
{
}
...
Grammar GrammarParser::parse()
{
    ...
    while (currentToken() != Token::Eof)
    {
        switch (currentToken())
        {
            case Token::Token: parseTokenBlock(); break;
            ...
            default:
                report_->syntaxError(
                    SourceLocation {}, "Unexpected token {}. Expecting a rule instead.", currentToken());
                consumeToken();
                abort();
                break;
        }
...
void GrammarParser::consumeToken(Token expectedToken)
{
    if (lexer_.currentToken() != expectedToken)
        report_->syntaxError(
            SourceLocation {}, "Expected token {} but got {}.", expectedToken, lexer_.currentToken());
...
{
    const auto i = find_if(begin(grammar_.explicitTerminals),
                           end(grammar_.explicitTerminals),
...
void GrammarParser::parseTokenBlock()
{
    ...
    regular::RuleList rules =
        regular::RuleParser { klexDef }.parseRules(); // TODO: currentLine, currentColumn
