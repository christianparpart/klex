...
{
	return any_of(initialStates_.begin(), initialStates_.end(),
...
{
	auto i = find_if(P.begin(), P.end(),
						  [s](const auto& p) { return find(p.begin(), p.end(), s) != p.end(); });
...
DFA DFAMinimizer::constructFromPartitions(const PartitionVec& P) const
{
	...
	for (const StateIdVec& p : P)
	{
		...
		const StateId q = p_i;
		DEBUG("Creating p{}: {} {}", p_i, dfa_.isAccepting(s) ? "accepting" : "rejecting",
