...
string charLiteral(Symbol ch)
{
    switch (ch)
    {
        ...
        case Symbols::Error: return fmt::format("{}", (int) ch);
        case ' ': return string { "' '" };
        case '\t': return string { "'\\t'" };
        case '\n': return string { "'\\n'" };
        case '\'': return string { "'\\''" };
...
void generateTableDefCxx(ostream& os,
                         const LexerDef& lexerDef,
...
void generateTokenDefCxx(ostream& os,
                         const RuleList& rules,
                         const string& tokenTypeName,
                         const string& machineTypeName,
                         const map<string, StateId>& initialStates)
{
...
optional<int> prepareAndParseCLI(Flags& flags, int argc, const char* argv[])
{
    ...
    flags.defineString("output-table",
                       't',
                       "FILE",
                       "Output file that will contain the compiled tables (use - to represent stderr)");
    flags.defineString("output-token",
                       'T',
                       "FILE",
                       "Output file that will contain the compiled tables (use - to represent stderr)");
    flags.defineString("table-name",
                       'n',
                       "IDENTIFIER",
                       "Symbol name for generated table (may include namespace).",
                       "lexerDef");
    flags.defineString("token-name",
                       'N',
                       "IDENTIFIER",
                       "Symbol name for generated token enum type (may include namespace).",
                       "Token");
    flags.defineString("machine-name",
                       'M',
                       "IDENTIFIER",
                       "Symbol name for generated machine enum type (must not include namespace).",
                       "Machine");
    flags.defineString("debug-dfa",
                       'x',
                       "DOT_FILE",
                       "Writes dot graph of final finite automaton. Use - to represent stdout.",
                       "");
    flags.defineBool(
        "debug-nfa", 'd', "Writes dot graph of non-deterministic finite automaton to stdout and exits.");
    ...
    {
        static string_view const title = "mklex - klex lexer generator\n"
                                         "(c) 2018 Christian Parpart <christian@parpart.family>\n"
                                         "\n";
...
int main(int argc, const char* argv[])
{
    ...
    {
        const Rule& shadowee = *find_if(
            rules.begin(), rules.end(), [&](const Rule& rule) { return rule.tag == overshadow.first; });
        const Rule& shadower = *find_if(
            rules.begin(), rules.end(), [&](const Rule& rule) { return rule.tag == overshadow.second; });
    ...
    if (string tokenFile = flags.getString("output-token"); tokenFile != "-")
    {
        ...
        generateTokenDefCxx(ofs,
                            rules,
                            flags.getString("token-name"),
                            flags.getString("machine-name"),
                            lexerDef.initialStates);
    }
    else
    {
        generateTokenDefCxx(cerr,
                            rules,
                            flags.getString("token-name"),
                            flags.getString("machine-name"),
                            lexerDef.initialStates);
