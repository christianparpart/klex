...
void consume(Lexer& lexer, Token t)
{
    if (lexer.token() != t)
        throw std::runtime_error { fmt::format(
            "Unexpected token {}. Expected {} instead.", lexer.token(), t) };
    ++lexer;
}
...
auto primaryExpr(Lexer& lexer)
{
    switch (lexer.token())
    {
        ...
        case Token::Minus: return -1 * primaryExpr(++lexer);
        case Token::RndOpen: {
            auto y = expr(++lexer);
        ...
        default:
            throw std::runtime_error { fmt::format(
                "Unexpected token {}. Expected primary expression instead.", lexer.token()) };
    }
}
...
auto mulExpr(Lexer& lexer)
{
    ...
    for (;;)
    {
        switch (lexer.token())
        {
            case Token::Mul: lhs = lhs * primaryExpr(++lexer); break;
            case Token::Div: lhs = lhs / primaryExpr(++lexer); break;
            default: return lhs;
        }
    }
}
...
auto addExpr(Lexer& lexer)
{
    ...
    for (;;)
    {
        switch (lexer.token())
        {
            case Token::Plus: lhs = lhs + mulExpr(++lexer); break;
            case Token::Minus: lhs = lhs - mulExpr(++lexer); break;
            default: return lhs;
        }
    }
}
